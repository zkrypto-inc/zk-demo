import type { LedgerRow, ProcessView, Tone } from "@/scenarios/types";

type Props = { view: Extract<ProcessView, { kind: "ledger" }> };
...
function Row({ row }: { row: LedgerRow }) {
  ...
  const rowBg = row.duplicate
    ? "bg-[var(--bad-soft)]/40"
    : "bg-transparent";

  return (
    <div
      className={`grid grid-cols-[110px_minmax(0,1fr)_120px_70px] items-center gap-2 border-b border-[var(--line)] px-2 py-2 text-[11px] last:border-b-0 ${rowBg}`}
    >
      <div className={`flex items-center gap-1 ${badge.color}`} style={{ paddingLeft: `${indent * 12}px` }}>
        {indent > 0 && <span className="text-[var(--muted)]">└</span>}
        <span className="font-mono text-[10px] font-semibold">{badge.label}</span>
      </div>

      <div className="min-w-0 truncate font-mono text-[10px] text-[var(--ink-2)]">
      ...
      <div className="text-right font-mono text-[10px] text-[var(--ink-2)]">
        {row.amount ?? "0"}
      </div>
      ...
      <div className="text-right">
        {row.blockable && !blocked && (
          <button
            type="button"
            className="rounded-md bg-[var(--bad)] px-2 py-1 text-[10px] font-semibold text-white"
          >
            지급 차단
          </button>
        )}
        {blocked && (
          <span className="rounded-md bg-[var(--bad-soft)] px-2 py-1 font-mono text-[10px] font-semibold text-[var(--bad)]">
            차단됨
          </span>
        )}
      </div>
...
export function LedgerProcessView({ view }: Props) {
  const { description, banner, caseId, rows, actionLog } = view;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--surface)]">
        <div className="grid grid-cols-[110px_minmax(0,1fr)_120px_70px] items-center gap-2 border-b border-[var(--line)] bg-[var(--surface-2)] px-2 py-2 text-[10px] font-semibold uppercase tracking-[0.04em] text-[var(--muted)]">
          <div>내부 트랜잭션</div>
          <div>전송자 → 수신자 / 메서드</div>
      ...
      {actionLog && actionLog.length > 0 && (
        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-2)] p-3">
          ...
          <div className="space-y-1 font-mono text-[10px] text-[var(--ink-2)]">
